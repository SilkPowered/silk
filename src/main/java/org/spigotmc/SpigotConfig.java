...
public class SpigotConfig
{
        ...
        for ( String name : section.getKeys( true ) )
        {
            if ( section.isInt( name ) )
            {
                try
                {
                    ...
                    if ( Registries.CUSTOM_STAT.get( key ) == null )
                    {
    ...
    private static void attributeMaxes()
    {
        ...
        ( (ClampedEntityAttribute) EntityAttributes.MAX_HEALTH ).maxValue = maxHealth;
        movementSpeed = getDouble( "settings.attribute.movementSpeed.max", movementSpeed );
        ( (ClampedEntityAttribute) EntityAttributes.MOVEMENT_SPEED ).maxValue = movementSpeed;
        attackDamage = getDouble( "settings.attribute.attackDamage.max", attackDamage );
        ( (ClampedEntityAttribute) EntityAttributes.ATTACK_DAMAGE ).maxValue = attackDamage;
    }
