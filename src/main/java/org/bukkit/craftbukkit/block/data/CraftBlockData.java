...
public class CraftBlockData implements BlockData {
    ...
    public String toString(Map<Property<?>, Comparable<?>> states) {
        StringBuilder stateString = new StringBuilder(Registries.BLOCK.b(state.getBlock()).toString());

        if (!states.isEmpty()) {
            stateString.append('[');
            stateString.append(states.entrySet().stream().map(State.PROPERTY_ENTRY_TO_STRING_FUNCTION).collect(Collectors.joining(",")));
            stateString.append(']');
        }
    ...
    static {
        ...
        register(net.minecraft.block.OxidizableSlabBlock.class, org.bukkit.craftbukkit.block.impl.CraftWeatheringCopperSlab::new);
    ...
    public static CraftBlockData newData(Material material, String data) {
        ...
        if (data != null) {
            try {
                ...
                if (block != null) {
                    data = Registries.BLOCK.b(block) + data;
                }

                StringReader reader = new StringReader(data);
                BlockArgumentParser.a arg = BlockArgumentParser.parseForBlock(Registries.BLOCK.asLookup(), reader, false);
    ...
    public boolean isFaceSturdy(BlockFace face, BlockSupport support) {
        ...
        return state.isSideSolid(EmptyBlockView.INSTANCE, BlockPos.ZERO, CraftBlock.blockFaceToNotch(face), CraftBlockSupport.toNMS(support));
    }

    @Override
    public Material getPlacementMaterial() {
        return CraftMagicNumbers.getMaterial(state.getBlock().k());
    }
