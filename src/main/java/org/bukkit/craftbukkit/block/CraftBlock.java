...
public class CraftBlock implements Block {
    ...
    public static Biome biomeBaseToBiome(net.minecraft.registry.Registry<net.minecraft.world.biome.Biome> registry, RegistryEntry<net.minecraft.world.biome.Biome> base) {
        return biomeBaseToBiome(registry, base.comp_349());
    }
    ...
    public static RegistryEntry<net.minecraft.world.biome.Biome> biomeToBiomeBase(net.minecraft.registry.Registry<net.minecraft.world.biome.Biome> registry, Biome bio) {
        ...
        return registry.getHolderOrThrow(RegistryKey.of(RegistryKeys.BIOME, CraftNamespacedKey.toMinecraft(bio.getKey())));
    }

    @Override
    public double getTemperature() {
        return world.getBiome(position).comp_349().getTemperature(position);
    }
    ...
    public boolean isBlockPowered() {
        return world.getMinecraftWorld().getReceivedStrongRedstonePower(position) > 0;
    }
    ...
    public boolean isBlockFaceIndirectlyPowered(BlockFace face) {
        int power = world.getMinecraftWorld().getEmittedRedstonePower(position, blockFaceToNotch(face));
    ...
    public int getBlockPower(BlockFace face) {
        ...
        int y = getY();
        int z = getZ();
        if ((face == BlockFace.DOWN || face == BlockFace.SELF) && world.isEmittingRedstonePower(new BlockPos(x, y - 1, z), Direction.DOWN)) power = getPower(power, world.a_(new BlockPos(x, y - 1, z)));
        if ((face == BlockFace.UP || face == BlockFace.SELF) && world.isEmittingRedstonePower(new BlockPos(x, y + 1, z), Direction.UP)) power = getPower(power, world.a_(new BlockPos(x, y + 1, z)));
        if ((face == BlockFace.EAST || face == BlockFace.SELF) && world.isEmittingRedstonePower(new BlockPos(x + 1, y, z), Direction.EAST)) power = getPower(power, world.a_(new BlockPos(x + 1, y, z)));
        if ((face == BlockFace.WEST || face == BlockFace.SELF) && world.isEmittingRedstonePower(new BlockPos(x - 1, y, z), Direction.WEST)) power = getPower(power, world.a_(new BlockPos(x - 1, y, z)));
        if ((face == BlockFace.NORTH || face == BlockFace.SELF) && world.isEmittingRedstonePower(new BlockPos(x, y, z - 1), Direction.NORTH)) power = getPower(power, world.a_(new BlockPos(x, y, z - 1)));
        if ((face == BlockFace.SOUTH || face == BlockFace.SELF) && world.isEmittingRedstonePower(new BlockPos(x, y, z + 1), Direction.SOUTH)) power = getPower(power, world.a_(new BlockPos(x, y, z + 1)));
    ...
    public boolean breakNaturally(ItemStack item) {
        ...
        if (block != Blocks.AIR && (item == null || !iblockdata.isToolRequired() || nmsItem.isSuitableFor(iblockdata))) {
            net.minecraft.block.Block.dropStacks(iblockdata, world.getMinecraftWorld(), position, world.getBlockEntity(position), null, nmsItem);
            result = true;
        }
    ...
    public boolean applyBoneMeal(BlockFace face) {
        ...
        ActionResult result = BoneMealItem.applyBonemeal(context);
    ...
    public Collection<ItemStack> getDrops(ItemStack item, Entity entity) {
        ...
        if (item == null || CraftBlockData.isPreferredTool(iblockdata, nms)) {
            return net.minecraft.block.Block.getDroppedStacks(iblockdata, (ServerWorld) world.getMinecraftWorld(), position, world.getBlockEntity(position), entity == null ? null : ((CraftEntity) entity).getHandle(), nms)
    ...
    public RayTraceResult rayTrace(Location start, Vector direction, double maxDistance, FluidCollisionMode fluidCollisionMode) {
        ...
        HitResult nmsHitResult = world.clip(new RaycastContext(startPos, endPos, RaycastContext.ShapeType.OUTLINE, CraftFluidCollisionMode.toNMS(fluidCollisionMode), null), position);
