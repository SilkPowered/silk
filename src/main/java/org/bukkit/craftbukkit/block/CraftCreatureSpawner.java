...
public class CraftCreatureSpawner extends CraftBlockEntityState<MobSpawnerBlockEntity> implements CreatureSpawner {
    ...
    public EntityType getSpawnedType() {
        MobSpawnerEntry spawnData = this.getSnapshot().getSpawner().nextSpawnData;
        if (spawnData == null) {
            return null;
        }

        Optional<net.minecraft.entity.EntityType<?>> type = net.minecraft.entity.EntityType.by(spawnData.getEntityToSpawn());
        return type.map(entityTypes -> EntityType.fromName(net.minecraft.entity.EntityType.getKey(entityTypes).getPath())).orElse(null);
    }

    @Override
    public void setSpawnedType(EntityType entityType) {
        if (entityType == null) {
            this.getSnapshot().getSpawner().spawnPotentials = DataPool.empty(); // need clear the spawnPotentials to avoid nextSpawnData being replaced later
            this.getSnapshot().getSpawner().nextSpawnData = new MobSpawnerEntry();
            return;
        }
        Preconditions.checkArgument(entityType != EntityType.UNKNOWN, "Can't spawn EntityType %s from mob spawners!", entityType);

        Random rand = (this.isPlaced()) ? this.getWorldHandle().getRandom() : Random.create();
        this.getSnapshot().setEntityId(net.minecraft.entity.EntityType.byString(entityType.getName()).get(), rand);
    }
    ...
    public String getCreatureTypeName() {
        ...
        Optional<net.minecraft.entity.EntityType<?>> type = net.minecraft.entity.EntityType.by(spawnData.getEntityToSpawn());
        return type.map(entityTypes -> net.minecraft.entity.EntityType.getKey(entityTypes).getPath()).orElse(null);
    }
