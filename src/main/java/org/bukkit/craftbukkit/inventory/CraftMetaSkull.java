...
class CraftMetaSkull extends CraftMetaItem implements SkullMeta {
    ...
    CraftMetaSkull(NbtCompound tag) {
        ...
        if (tag.contains(BLOCK_ENTITY_TAG.NBT, CraftMagicNumbers.NBT.TAG_COMPOUND)) {
            NbtCompound nbtTagCompound = tag.getCompound(BLOCK_ENTITY_TAG.NBT).copy();
            if (nbtTagCompound.contains(NOTE_BLOCK_SOUND.NBT, 8)) {
                this.noteBlockSound = Identifier.tryParse(nbtTagCompound.getString(NOTE_BLOCK_SOUND.NBT));
            }
        }
    }
    ...
    void deserializeInternal(NbtCompound tag, Object context) {
        ...
        if (tag.contains(SKULL_PROFILE.NBT, CraftMagicNumbers.NBT.TAG_COMPOUND)) {
            ...
            this.setProfile(NbtHelper.toGameProfile(skullTag));
        }
