...
public final class CraftItemFactory implements ItemFactory {
    ...
    public ItemStack createItemStack(String input) throws IllegalArgumentException {
        try {
            ItemStringReader.a arg = ItemStringReader.parseForItem(Registries.ITEM.asLookup(), new StringReader(input));

            Item item = arg.item().comp_349();
            net.minecraft.item.ItemStack nmsItemStack = new net.minecraft.item.ItemStack(item);

            NbtCompound nbt = arg.nbt();
            if (nbt != null) {
                nmsItemStack.setNbt(nbt);
            }
    ...
    public Material getSpawnEgg(EntityType type) {
        ...
        net.minecraft.entity.EntityType<?> nmsType = Registries.ENTITY_TYPE.a(CraftNamespacedKey.toMinecraft(type.getKey()));
        Item nmsItem = SpawnEggItem.forEntity(nmsType);
