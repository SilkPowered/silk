...
import cx.rain.silk.mixins.statics.item.BlockItemStatics;
...
class CraftMetaItem implements ItemMeta, Damageable, Repairable, BlockDataMeta {
    ...
    final Map<String, NbtElement> unhandledTags = new HashMap<>(); // Visible for testing only
    ...
    void applyToItem(NbtCompound itemTag) {
        if (hasDisplayName()) {
            setDisplayTag(itemTag, NAME.NBT, NbtString.of(displayName));
        }
        if (hasLocalizedName()) {
            setDisplayTag(itemTag, LOCNAME.NBT, NbtString.of(locName));
        }
    ...
    NbtList createStringList(List<String> list) {
        if (list == null) {
            return null;
        }
        ...
        for (String value : list) {
            ...
            tagList.add(NbtString.of(version <= 0 || version >= 1803 ? value : CraftChatMessage.fromJSONComponent(value))); // SPIGOT-4935
        }
    ...
    static void applyModifiers(Multimap<Attribute, AttributeModifier> modifiers, NbtCompound tag, ItemMetaKey key) {
        ...
        for (Map.Entry<Attribute, AttributeModifier> entry : modifiers.entries()) {
            ...
            EntityAttributeModifier nmsModifier = CraftAttributeInstance.convert(entry.getValue());
    ...
    public BlockData getBlockData(Material material) {
        BlockState defaultData = CraftMagicNumbers.getBlock(material).getDefaultState();
        return CraftBlockData.fromData((hasBlockData()) ? BlockItemStatics.getBlockState(defaultData, blockData) : defaultData);
    }
    ...
    static void safelyAdd(Iterable<?> addFrom, Collection<String> addTo, boolean possiblyJsonInput) {
        ...
        for (Object object : addFrom) {
            if (!(object instanceof String)) {
                ...
                addTo.add(CraftChatMessage.toJSON(Text.empty()));
            } else {
