...
class CraftMetaLeatherArmor extends CraftMetaItem implements LeatherArmorMeta {
    ...
    CraftMetaLeatherArmor(NbtCompound tag) {
        super(tag);
        readColor(this, tag);
    }
    ...
    static void readColor(LeatherArmorMeta meta, NbtCompound tag) {
    ...
    static void applyColor(LeatherArmorMeta meta, NbtCompound tag) {
        if (hasColor(meta)) {
            ((CraftMetaItem) meta).setDisplayTag(tag, COLOR.NBT, NbtInt.of(meta.getColor().asRGB()));
        }
    }
