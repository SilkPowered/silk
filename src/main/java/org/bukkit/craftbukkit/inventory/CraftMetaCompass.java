...
public class CraftMetaCompass extends CraftMetaItem implements CompassMeta {
    ...
    public Location getLodestone() {
        ...
        ServerWorld worldServer = key.isPresent() ? IMinecraftServerMixin.getServer().getWorld(key.get()) : null;
        World world = worldServer != null ? ((IWorldMixin) worldServer).getWorld() : null;
    ...
    public void setLodestone(Location lodestone) {
        ...
        if (lodestone == null) {
            ...
            RegistryKey<net.minecraft.world.World> key = ((CraftWorld) lodestone.getWorld()).getHandle().toServerWorld().getRegistryKey();
            DataResult<NbtElement> dataresult = net.minecraft.world.World.CODEC.encodeStart(NbtOps.INSTANCE, key);
