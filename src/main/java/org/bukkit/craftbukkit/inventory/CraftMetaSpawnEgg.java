...
public class CraftMetaSpawnEgg extends CraftMetaItem implements SpawnEggMeta {
    ...
    CraftMetaSpawnEgg(NbtCompound tag) {
        super(tag);
    ...
    void deserializeInternal(NbtCompound tag, Object context) {
        ...
        if (tag.contains(ENTITY_TAG.NBT)) {
            ...
            if (!entityTag.isEmpty()) {
                ...
                // entityTag = (NbtCompound) MinecraftServer.getServer().dataConverterManager.update(DataConverterTypes.ENTITY, new Dynamic(DynamicOpsNBT.a, entityTag), -1, CraftMagicNumbers.DATA_VERSION).getValue();
            }

            // See if we can read a converted ID tag
            if (entityTag.contains(ENTITY_ID.NBT)) {
                this.spawnedType = EntityType.fromName(new Identifier(entityTag.getString(ENTITY_ID.NBT)).getPath());
            }
        }
    }
