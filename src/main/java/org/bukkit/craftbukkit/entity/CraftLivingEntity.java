...
public class CraftLivingEntity extends CraftEntity implements LivingEntity {
    ...
    public void setHealth(double health) {
        ...
        if (health == 0) {
            getHandle().onDeath(getHandle().getDamageSources().generic());
        }
    }
    ...
    public void setArrowsInBody(int count) {
        ...
        getHandle().getDataTracker().set(net.minecraft.entity.LivingEntity.DATA_ARROW_COUNT_ID, count);
    }
    ...
    public void damage(double amount, org.bukkit.entity.Entity source) {
        ...
        if (source instanceof HumanEntity) {
            ...
            reason = getHandle().getDamageSources().mobAttack(((CraftLivingEntity) source).getHandle());
        }
    ...
    public int getNoActionTicks() {
        return getHandle().getDespawnCounter();
    }

    @Override
    public void setNoActionTicks(int ticks) {
        Preconditions.checkArgument(ticks >= 0, "ticks must be >= 0");
        getHandle().setDespawnCounter(ticks);
    }
    ...
    public boolean addPotionEffect(PotionEffect effect, boolean force) {
        getHandle().addEffect(new StatusEffectInstance(StatusEffect.byRawId(effect.getType().getId()), effect.getDuration(), effect.getAmplifier(), effect.isAmbient(), effect.hasParticles()), EntityPotionEffectEvent.Cause.PLUGIN);
        return true;
    }
    ...
    public void removePotionEffect(PotionEffectType type) {
        getHandle().removeEffect(StatusEffect.byRawId(type.getId()), EntityPotionEffectEvent.Cause.PLUGIN);
    }

    @Override
    public Collection<PotionEffect> getActivePotionEffects() {
        List<PotionEffect> effects = new ArrayList<PotionEffect>();
        for (StatusEffectInstance handle : getHandle().activeEffects.values()) {
    ...
    public <T extends Projectile> T launchProjectile(Class<? extends T> projectile, Vector velocity) {
        ...
        if (Snowball.class.isAssignableFrom(projectile)) {
            ...
            ((ThrownEntity) launch).setVelocity(getHandle(), getHandle().getPitch(), getHandle().getYaw(), 0.0F, 1.5F, 1.0F); // ItemSnowball
        } else if (Egg.class.isAssignableFrom(projectile)) {
            launch = new EggEntity(world, getHandle());
            ((ThrownEntity) launch).setVelocity(getHandle(), getHandle().getPitch(), getHandle().getYaw(), 0.0F, 1.5F, 1.0F); // ItemEgg
        } else if (EnderPearl.class.isAssignableFrom(projectile)) {
            launch = new EnderPearlEntity(world, getHandle());
            ((ThrownEntity) launch).setVelocity(getHandle(), getHandle().getPitch(), getHandle().getYaw(), 0.0F, 1.5F, 1.0F); // ItemEnderPearl
            ...
            ((PersistentProjectileEntity) launch).setVelocity(getHandle(), getHandle().getPitch(), getHandle().getYaw(), 0.0F, 3.0F, 1.0F); // ItemBow
            ...
            ((ThrownEntity) launch).setVelocity(getHandle(), getHandle().getPitch(), getHandle().getYaw(), -20.0F, 0.5F, 1.0F); // ItemSplashPotion
        } else if (ThrownExpBottle.class.isAssignableFrom(projectile)) {
            launch = new ExperienceBottleEntity(world, getHandle());
            ((ThrownEntity) launch).setVelocity(getHandle(), getHandle().getPitch(), getHandle().getYaw(), -20.0F, 0.7F, 1.0F); // ItemExpBottle
            ...
            if (SmallFireball.class.isAssignableFrom(projectile)) {
                ...
                launch = new FireballEntity(world, getHandle(), direction.getX(), direction.getY(), direction.getZ(), 1);
            }
    ...
    public boolean getRemoveWhenFarAway() {
        return getHandle() instanceof MobEntity && !((MobEntity) getHandle()).isPersistent();
    }

    @Override
    public void setRemoveWhenFarAway(boolean remove) {
        if (getHandle() instanceof MobEntity) {
            ((MobEntity) getHandle()).setPersistenceRequired(!remove);
        }
    }
    ...
    public boolean setLeashHolder(Entity holder) {
        if (getHandle().generation || (getHandle() instanceof WitherEntity) || !(getHandle() instanceof MobEntity)) {
            return false;
        }
    ...
    public void attack(Entity target) {
        ...
        if (getHandle() instanceof PlayerEntity) {
            ((PlayerEntity) getHandle()).attack(((CraftEntity) target).getHandle());
        } else {
    ...
    public void playHurtAnimation(float yaw) {
        if (getHandle().getWorld() instanceof ServerWorld world) {
            /*
            ...
            DamageTiltS2CPacket packet = new DamageTiltS2CPacket(getEntityId(), actualYaw);

            world.getChunkManager().sendToNearbyPlayers(getHandle(), packet);
        }
    }
    ...
    public <T> T getMemory(MemoryKey<T> memoryKey) {
        return (T) getHandle().getBrain().getOptionalRegisteredMemory(CraftMemoryKey.fromMemoryKey(memoryKey)).map(CraftMemoryMapper::fromNms).orElse(null);
    }
    ...
    public Sound getHurtSound() {
        SoundEvent sound = getHandle().getHurtSound0(getHandle().getDamageSources().generic());
    ...
    public EntityCategory getCategory() {
        EntityGroup type = getHandle().getGroup(); // Not actually an enum?

        if (type == EntityGroup.UNDEFINED) {
            return EntityCategory.NONE;
        } else if (type == EntityGroup.UNDEAD) {
            return EntityCategory.UNDEAD;
        } else if (type == EntityGroup.ARTHROPOD) {
            return EntityCategory.ARTHROPOD;
        } else if (type == EntityGroup.ILLAGER) {
            return EntityCategory.ILLAGER;
        } else if (type == EntityGroup.WATER) {
