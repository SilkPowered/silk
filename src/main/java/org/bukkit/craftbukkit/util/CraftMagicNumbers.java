...
public final class CraftMagicNumbers implements UnsafeValues {
    ...
    private static final BiMap<net.minecraft.fluid.Fluid, Fluid> FLUIDTYPE_FLUID = HashBiMap.create();
    private static final Map<Material, Item> MATERIAL_ITEM = new HashMap<>();
    private static final Map<Material, Block> MATERIAL_BLOCK = new HashMap<>();

    static {
        for (Block block : Registries.BLOCK) {
            BLOCK_MATERIAL.put(block, Material.getMaterial(Registries.BLOCK.b(block).getPath().toUpperCase(Locale.ROOT)));
        }

        for (Item item : Registries.ITEM) {
            ITEM_MATERIAL.put(item, Material.getMaterial(Registries.ITEM.b(item).getPath().toUpperCase(Locale.ROOT)));
        }

        for (net.minecraft.fluid.Fluid fluidType : Registries.FLUID) {
            Fluid fluid = Registry.FLUID.get(CraftNamespacedKey.fromMinecraft(Registries.FLUID.b(fluidType)));
    ...
    private static File getBukkitDataPackFolder() {
        return new File(MinecraftServer.getServer().getSavePath(WorldSavePath.DATAPACK_DIR).toFile(), "bukkit");
    }
    ...
    public Advancement loadAdvancement(NamespacedKey key, String advancement) {
        ...
        net.minecraft.advancement.Advancement.Builder nms = net.minecraft.advancement.Advancement.Builder.fromJson(jsonobject, new AdvancementEntityPredicateDeserializer(minecraftkey, MinecraftServer.getServer().getLootManager()));
        if (nms != null) {
            MinecraftServer.getServer().getAdvancementLoader().advancements.load(Maps.newHashMap(Collections.singletonMap(minecraftkey, nms)));
    ...
    public Multimap<Attribute, AttributeModifier> getDefaultAttributeModifiers(Material material, EquipmentSlot slot) {
        ...
        Multimap<EntityAttribute, net.minecraft.entity.attribute.EntityAttributeModifier> nmsDefaultAttributes = getItem(material).getAttributeModifiers(CraftEquipmentSlot.getNMS(slot));
        for (Entry<EntityAttribute, net.minecraft.entity.attribute.EntityAttributeModifier> mapEntry : nmsDefaultAttributes.entries()) {
            Attribute attribute = CraftAttributeMap.fromMinecraft(Registries.ATTRIBUTE.getId(mapEntry.getKey()).toString());
    ...
    public String getTranslationKey(EntityType entityType) {
        ...
        return net.minecraft.entity.EntityType.get(entityType.getName()).map(net.minecraft.entity.EntityType::getTranslationKey).orElseThrow();
    }

    @Override
    public String getTranslationKey(ItemStack itemStack) {
        net.minecraft.item.ItemStack nmsItemStack = CraftItemStack.asNMSCopy(itemStack);
        return nmsItemStack.getItem().getTranslationKey(nmsItemStack);
    }
