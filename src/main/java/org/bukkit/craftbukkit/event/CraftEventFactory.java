...
public class CraftEventFactory {
    ...
    public static boolean callPlayerSignOpenEvent(PlayerEntity player, SignBlockEntity tileEntitySign, boolean front, PlayerSignOpenEvent.Cause cause) {
        final Block block = CraftBlock.at(tileEntitySign.getWorld(), tileEntitySign.getPos());
    ...
    public static Either<PlayerEntity.SleepFailureReason, Unit> callPlayerBedEnterEvent(PlayerEntity player, BlockPos bed, Either<PlayerEntity.SleepFailureReason, Unit> nmsBedResult) {
        ...
        PlayerBedEnterEvent event = new PlayerBedEnterEvent((Player) player.getBukkitEntity(), CraftBlock.at(player.getWorld(), bed), bedEnterResult);
        Bukkit.getServer().getPluginManager().callEvent(event);
     ...
     */
    public static PlayerHarvestBlockEvent callPlayerHarvestBlockEvent(World world, BlockPos blockposition, PlayerEntity who, Hand enumhand, List<ItemStack> itemsToHarvest) {
     ...
     */
    public static PlayerBucketEntityEvent callPlayerFishBucketEvent(net.minecraft.entity.LivingEntity fish, PlayerEntity entityHuman, ItemStack originalBucket, ItemStack entityBucket, Hand enumhand) {
        Player player = (Player) entityHuman.getBukkitEntity();
        EquipmentSlot hand = CraftEquipmentSlot.getHand(enumhand);

        PlayerBucketEntityEvent event;
        if (fish instanceof FishEntity) {
    ...
    public static boolean handleBellRingEvent(World world, BlockPos position, Direction direction, Entity entity) {
    ...
    public static Stream<net.minecraft.entity.LivingEntity> handleBellResonateEvent(World world, BlockPos position, List<LivingEntity> bukkitEntities) {
        Block block = CraftBlock.at(world, position);
    ...
    public static EntityPlaceEvent callEntityPlaceEvent(ItemUsageContext itemactioncontext, Entity entity) {
        return callEntityPlaceEvent(itemactioncontext.getWorld(), itemactioncontext.getBlockPos(), itemactioncontext.getSide(), itemactioncontext.getPlayer(), entity, itemactioncontext.getHand());
    }
    ...
    public static EntityPlaceEvent callEntityPlaceEvent(World world, BlockPos clickPosition, Direction clickedFace, PlayerEntity human, Entity entity, Hand enumhand) {
        ...
        entity.getWorld().getCraftServer().getPluginManager().callEvent(event);
     ...
     * Bucket methods
     */
    public static PlayerBucketEmptyEvent callPlayerBucketEmptyEvent(ServerWorld world, PlayerEntity who, BlockPos changed, BlockPos clicked, Direction clickedFace, ItemStack itemInHand, Hand enumhand) {
    ...
    public static PlayerBucketFillEvent callPlayerBucketFillEvent(ServerWorld world, PlayerEntity who, BlockPos changed, BlockPos clicked, Direction clickedFace, ItemStack itemInHand, net.minecraft.item.Item bucket, Hand enumhand) {
        return (PlayerBucketFillEvent) getPlayerBucketEvent(true, world, who, clicked, changed, clickedFace, itemInHand, bucket, enumhand);
    }

    private static PlayerEvent getPlayerBucketEvent(boolean isFilling, ServerWorld world, PlayerEntity who, BlockPos changed, BlockPos clicked, Direction clickedFace, ItemStack itemstack, net.minecraft.item.Item item, Hand enumhand) {
        Player player = (Player) who.getBukkitEntity();
    ...
    public static PlayerInteractEvent callPlayerInteractEvent(PlayerEntity who, Action action, BlockPos position, Direction direction, ItemStack itemstack, boolean cancelledBlock, Hand hand, Vec3d targetPos) {
        Player player = (who == null) ? null : (Player) who.getBukkitEntity();
        CraftItemStack itemInHand = CraftItemStack.asCraftMirror(itemstack);

        Vector clickedPos = null;
        if (position != null && targetPos != null) {
            clickedPos = CraftVector.toBukkit(targetPos.subtract(Vec3d.of(position)));
        }
        ...
        PlayerInteractEvent event = new PlayerInteractEvent(player, action, itemInHand, blockClicked, blockFace, (hand == null) ? null : ((hand == Hand.OFF_HAND) ? EquipmentSlot.OFF_HAND : EquipmentSlot.HAND), clickedPos);
        if (cancelledBlock) {
     ...
     * BlockDamageEvent
     */
    public static BlockDamageEvent callBlockDamageEvent(ServerPlayerEntity who, BlockPos pos, ItemStack itemstack, boolean instaBreak) {
        Player player = who.getBukkitEntity();
        CraftItemStack itemInHand = CraftItemStack.asCraftMirror(itemstack);

        Block blockClicked = CraftBlock.at(who.getWorld(), pos);
    ...
    public static BlockDamageAbortEvent callBlockDamageAbortEvent(ServerPlayerEntity who, BlockPos pos, ItemStack itemstack) {
        Player player = who.getBukkitEntity();
        CraftItemStack itemInHand = CraftItemStack.asCraftMirror(itemstack);
    ...
    public static boolean doEntityAddEventCalling(World world, Entity entity, SpawnReason spawnReason) {
        ...
        if (entity instanceof net.minecraft.entity.LivingEntity && !(entity instanceof ServerPlayerEntity)) {
            ...
            if (spawnReason != SpawnReason.CUSTOM) {
                if (isAnimal && !world.getWorld().getAllowAnimals() || isMonster && !world.getWorld().getAllowMonsters() || isNpc && !world.getCraftServer().getServer().X()) {
                    entity.discard();
                    return false;
                }
            }
        ...
        // Spigot start
            ...
            if (radius > 0) {
                List<Entity> entities = world.getOtherEntities(entity, entity.cE().expand(radius, radius, radius));
    ...
    public static boolean handleBlockSpreadEvent(WorldAccess world, BlockPos source, BlockPos target, net.minecraft.block.BlockState block, int flag) {
    ...
    private static EntityDamageEvent handleEntityDamageEvent(Entity entity, DamageSource source, Map<DamageModifier, Double> modifiers, Map<DamageModifier, Function<? super Double, Double>> modifierFunctions, boolean cancelled) {
        if (source.isIn(DamageTypeTags.IS_EXPLOSION)) {
            ...
            return event;
        } else if (source.getAttacker() != null || source.getSource() != null) {
            Entity damager = source.getAttacker();
            DamageCause cause = (source.isSweep()) ? DamageCause.ENTITY_SWEEP_ATTACK : DamageCause.ENTITY_ATTACK;

            if (source.isIndirect() && source.getSource() != null) {
                damager = source.getSource();
            }
            ...
            } else if (source.isOf(DamageTypes.THORNS)) {
        ...
        } else if (source.isOf(DamageTypes.FELL_OUT_OF_WORLD)) {
            ...
            Block damager = blockDamage;
            if (source.isOf(DamageTypes.CACTUS) || source.isOf(DamageTypes.SWEET_BERRY_BUSH) || source.isOf(DamageTypes.STALAGMITE) || source.isOf(DamageTypes.FALLING_STALACTITE) || source.isOf(DamageTypes.FALLING_ANVIL)) {
                ...
                throw new IllegalStateException(String.format("Unhandled damage of %s by %s from %s", entity, damager, source.getName()));
            }
            ...
            entityDamage = null;
            if (source.isOf(DamageTypes.FALLING_STALACTITE) || source.isOf(DamageTypes.FALLING_BLOCK) || source.isOf(DamageTypes.FALLING_ANVIL)) {
                cause = DamageCause.FALLING_BLOCK;
            } else if (damager instanceof LightningStrike) {
                cause = DamageCause.LIGHTNING;
            } else if (source.isOf(DamageTypes.FALL)) {
                cause = DamageCause.FALL;
            } else if (source.isOf(DamageTypes.DRAGON_BREATH)) {
                cause = DamageCause.DRAGON_BREATH;
            } else if (source.isOf(DamageTypes.MAGIC)) {
                cause = DamageCause.MAGIC;
            } else {
                throw new IllegalStateException(String.format("Unhandled damage of %s by %s from %s", entity, damager.getHandle(), source.getName()));
            }
        ...
        } else if (source.isOf(DamageTypes.CRAMMING)) {
            cause = DamageCause.CRAMMING;
        } else if (source.isOf(DamageTypes.DRY_OUT)) {
        ...
        } else if (source.isOf(DamageTypes.OUTSIDE_BORDER)) {
        ...
        throw new IllegalStateException(String.format("Unhandled damage of %s from %s", entity, source.getName()));
    }
    ...
    public static EntityDamageEvent handleLivingEntityDamageEvent(Entity damagee, DamageSource source, double rawDamage, double hardHatModifier, double blockingModifier, double armorModifier, double resistanceModifier, double magicModifier, double absorptionModifier, Function<Double, Double> hardHat, Function<Double, Double> blocking, Function<Double, Double> armor, Function<Double, Double> resistance, Function<Double, Double> magic, Function<Double, Double> absorption) {
        ...
        if (source.isOf(DamageTypes.FALLING_BLOCK) || source.isOf(DamageTypes.FALLING_ANVIL)) {
    ...
    public static PlayerExpCooldownChangeEvent callPlayerXpCooldownEvent(PlayerEntity entity, int newCooldown, PlayerExpCooldownChangeEvent.ChangeReason changeReason) {
        Player player = (Player) entity.getBukkitEntity();
    ...
    public static PlayerItemMendEvent callPlayerItemMendEvent(PlayerEntity entity, ExperienceOrbEntity orb, net.minecraft.item.ItemStack nmsMendedItem, net.minecraft.entity.EquipmentSlot slot, int repairAmount) {
        Player player = (Player) entity.getBukkitEntity();
    ...
    public static boolean callEntityChangeBlockEvent(Entity entity, BlockPos position, net.minecraft.block.BlockState newBlock) {
        return callEntityChangeBlockEvent(entity, position, newBlock, false);
    }

    public static boolean callEntityChangeBlockEvent(Entity entity, BlockPos position, net.minecraft.block.BlockState newBlock, boolean cancelled) {
        Block block = entity.getWorld().getWorld().getBlockAt(position.getX(), position.getY(), position.getZ());
    ...
    public static EntityBreakDoorEvent callEntityBreakDoorEvent(Entity entity, BlockPos pos) {
        ...
        Block block = CraftBlock.at(entity.getWorld(), pos);
    ...
    public static ScreenHandler callInventoryOpenEvent(ServerPlayerEntity player, ScreenHandler container, boolean cancelled) {
        if (player.containerMenu != player.inventoryMenu) { // fire INVENTORY_CLOSE if one already open
            player.connection.a(new CloseHandledScreenC2SPacket(player.containerMenu.containerId));
        }

        CraftServer server = player.getWorld().getCraftServer();
    ...
    public static ProjectileHitEvent callProjectileHitEvent(Entity entity, HitResult position) {
        ...
        if (position.getType() == HitResult.Type.BLOCK) {
            ...
            hitBlock = CraftBlock.at(entity.getWorld(), positionBlock.getBlockPos());
    ...
    public static BlockIgniteEvent callBlockIgniteEvent(World world, BlockPos pos, Entity igniter) {
        ...
        if (igniter instanceof ProjectileEntity) {
            Entity shooter = ((ProjectileEntity) igniter).v();
            if (shooter != null) {
    ...
    public static void handleInventoryCloseEvent(PlayerEntity human) {
        ...
        human.getWorld().getCraftServer().getPluginManager().callEvent(event);
    ...
    public static ItemStack handleEditBookEvent(ServerPlayerEntity player, int itemInHandIndex, ItemStack itemInHand, ItemStack newBookItem) {
        ...
        player.getWorld().getCraftServer().getPluginManager().callEvent(editBookEvent);
    ...
    public static PlayerUnleashEntityEvent callPlayerUnleashEntityEvent(MobEntity entity, PlayerEntity player, Hand enumhand) {
    ...
    public static PlayerLeashEntityEvent callPlayerLeashEntityEvent(MobEntity entity, Entity leashHolder, PlayerEntity player, Hand enumhand) {
    ...
    public static Cancellable handleStatisticsIncrease(PlayerEntity entityHuman, net.minecraft.stat.Stat<?> statistic, int current, int newValue) {
        ...
        entityHuman.getWorld().getCraftServer().getPluginManager().callEvent(event);
    ...
    public static FireworkExplodeEvent callFireworkExplodeEvent(FireworkRocketEntity firework) {
        ...
        firework.getWorld().getCraftServer().getPluginManager().callEvent(event);
        return event;
    }
    ...
    public static SpawnerSpawnEvent callSpawnerSpawnEvent(Entity spawnee, BlockPos pos) {
        ...
        BlockState state = CraftBlock.at(spawnee.getWorld(), pos).getState();
    ...
    public static AreaEffectCloudApplyEvent callAreaEffectCloudApplyEvent(AreaEffectCloudEntity cloud, List<LivingEntity> entities) {
        ...
        cloud.getWorld().getCraftServer().getPluginManager().callEvent(event);
        return event;
    }
    ...
    public static EntityBreedEvent callEntityBreedEvent(net.minecraft.entity.LivingEntity child, net.minecraft.entity.LivingEntity mother, net.minecraft.entity.LivingEntity father, net.minecraft.entity.LivingEntity breeder, ItemStack bredWith, int experience) {
        ...
        child.getWorld().getCraftServer().getPluginManager().callEvent(event);
        return event;
    }
    ...
    public static LootGenerateEvent callLootGenerateEvent(Inventory inventory, LootTable lootTable, LootContext lootInfo, List<ItemStack> loot, boolean plugin) {
        ...
        NamespacedKey key = CraftNamespacedKey.fromMinecraft(world.getHandle().n().getLootManager().lootTableToKey.get(lootTable));
    ...
    public static boolean callStriderTemperatureChangeEvent(StriderEntity strider, boolean shivering) {
    ...
    public static void callEntitiesLoadEvent(World world, ChunkPos coords, List<Entity> entities) {
        ...
        EntitiesLoadEvent event = new EntitiesLoadEvent(new CraftChunk((ServerWorld) world, coords.x, coords.z), bukkitEntities);
        Bukkit.getPluginManager().callEvent(event);
    }

    public static void callEntitiesUnloadEvent(World world, ChunkPos coords, List<Entity> entities) {
        List<org.bukkit.entity.Entity> bukkitEntities = Collections.unmodifiableList(entities.stream().map(Entity::getBukkitEntity).collect(Collectors.toList()));
        EntitiesUnloadEvent event = new EntitiesUnloadEvent(new CraftChunk((ServerWorld) world, coords.x, coords.z), bukkitEntities);
        Bukkit.getPluginManager().callEvent(event);
    }
    ...
    public static PlayerRecipeBookClickEvent callRecipeBookClickEvent(ServerPlayerEntity player, Recipe recipe, boolean shiftClick) {
    ...
    public static EntityTeleportEvent callEntityTeleportEvent(Entity nmsEntity, double x, double y, double z) {
        ...
        Location to = new Location(entity.getWorld(), x, y, z, nmsEntity.getYaw(), nmsEntity.getPitch());
