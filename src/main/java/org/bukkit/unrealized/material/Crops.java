...
import org.bukkit.unrealized.CropState;
...
public class Crops extends MaterialData {
    ...
    public CropState getState() {
        switch (getItemType()) {
            case Material.LEGACY_CROPS:
            case Material.LEGACY_CARROT:
            case Material.LEGACY_POTATO:
                // Mask the data just in case top bit set
                return CropState.getByData((byte) (getData() & 0x7));
            case Material.LEGACY_BEETROOT_BLOCK:
            case Material.LEGACY_NETHER_WARTS:
