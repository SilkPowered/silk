...
package cx.rain.silk.loader.patch;
...
public final class SilkBrandingPath extends GamePatch {
	public static final String INTERNAL_NAME = SilkBrandingPath.class.getName().replace('.', '/');
	...
	public static String insertBranding(final String brand) {
		...
		return brand.equals("vanilla") ? "silk" : brand + ',' + "silk";
	}
			...
			if (node.name.equals("getClientModName") || node.name.equals("getServerModName") && node.desc.endsWith(")Ljava/lang/String;")) {
				...
				while (it.hasNext()) {
					if (it.next().getOpcode() == Opcodes.ARETURN) {
						it.previous();
						it.add(new MethodInsnNode(Opcodes.INVOKESTATIC, INTERNAL_NAME, "insertBranding", "(Ljava/lang/String;)Ljava/lang/String;", false));
						it.next();
					}
				}
